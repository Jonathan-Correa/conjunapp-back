...
def create_access_token(subject: UUID, audience: TokenAudience) -> str:
    ...
    payload = {"sub": str(subject), "aud": audience, "exp": expires_at}
...
def decode_access_token(token: str, audience: TokenAudience) -> dict:
    settings = get_settings()
    try:
        return jwt.decode(token, settings.jwt_secret_key, algorithms=[settings.jwt_algorithm], audience=audience)
